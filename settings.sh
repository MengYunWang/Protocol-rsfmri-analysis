...
if [ "$FSL_FIX_MATLAB_MODE" -eq 0 ]; then
	...
	if [ -z "${FSL_FIX_MCRROOT}" ]; then
		FSL_FIX_MCRROOT="/Applications/MATLAB/MATLAB_Runtime"
...
if [ "$FSL_FIX_MATLAB_MODE" -eq 1 ]; then
	...
	if [ -z "${FSL_FIX_MATLAB_ROOT}" ]; then
		FSL_FIX_MATLAB_ROOT="/Applications/MATLAB_R2021b"
...
if [ -z "${FSL_FIX_CIFTIRW}" ]; then
	FSL_FIX_CIFTIRW='/Users/wang/Documents/MATLAB/toolbox/cifti-matlab';
fi
